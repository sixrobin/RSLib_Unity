namespace RSLib.Unity.DungeonGenerator
{
    ...
    public class MapDataGenerator
    {
        ...
        private void FillBlankRooms()
        {
            ...
            {
                foreach (RoomType r in RoomTypeUtilities.SINGLE_OPENINGS)
                {
                    if (!_roomsTypes[fillRooms[i].x, fillRooms[i].y].HasOpening(r))
                        continue;

                    int openedToX = r switch
                    {
                        RoomType.L => fillRooms[i].x - 1,
                        RoomType.R => fillRooms[i].x + 1,
                        _ => fillRooms[i].x
                    };
                    
                    int openedToY = r switch
                    {
                        RoomType.T => fillRooms[i].y - 1,
                        RoomType.B => fillRooms[i].y + 1,
                        _ => fillRooms[i].y
                    };
