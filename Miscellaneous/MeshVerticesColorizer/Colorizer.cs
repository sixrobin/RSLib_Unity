namespace RSLib.Unity.MeshVerticesColorizer
{
	...
	public class Colorizer : MonoBehaviour
	{
		...
		[SerializeField]
		private float _travelDistToUpdate = 1f;
		[SerializeField]
		private float _detectionDist = 1000f;
		[SerializeField]
		private float _colorRadius = 3f;
		[SerializeField]
		...
		private void LookForColorableMesh()
		{
			...
			if (this._alreadyKnownColorables.TryGetValue(hitTransform, out ColorableMesh knownColorable))
			{
				if (_showDebug)
					Debug.Log($"{nameof(Colorizer)}: Detected an already known ColorableMesh {hitTransform.name}. Distance : {hit.distance}.", gameObject);
				
				knownColorable.ColorAtWorldPosition(hit.point, _colorRadius, _paintColor);
				return;
			}
