namespace RSLib.ImageEffects
{
	...
	public class ShadowCastingLight : MonoBehaviour
    {
		...
		[Header("DETECTION")]
		[SerializeField]
		private LayerMask _obstaclesMask = 0;

		[Tooltip("Colliders to consider, whatever their start range from the light.")]
		[SerializeField]
		private Collider2D[] _autoConsidered = null;

		[Header("SETTINGS")]
		[SerializeField, Range(1f, 100f)]
		private float _range = 20;
		[SerializeField, Range(0.001f, 0.1f)]
		private float _boxPrecision = 0.001f;
		[SerializeField, Range(0.001f, 0.1f)]
		private float _circlePrecision = 0.01f;

		[Tooltip ("Circles that are close to the light will require higher resolution.")]
		[SerializeField]
		private CircleResolution _circlesResolution = CircleResolution.SIXTEEN;

        [Header("DEBUG")]
        [SerializeField]
        private bool _showRange = true;
        [SerializeField]
        private bool _showCircleResolution = false;
        [SerializeField]
        ...
        private ObstacleData GetCircleData(CircleCollider2D circle)
		{
			...
			{
				_allCirclesPoints ??= Array.Empty<Vector2>();
