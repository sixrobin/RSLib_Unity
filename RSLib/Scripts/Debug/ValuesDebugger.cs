namespace RSLib.Debug
{
    ...
    public sealed class ValuesDebugger : Framework.Singleton<ValuesDebugger>
    {
        ...
        private Dictionary<Anchor, Dictionary<string, object>> _values = new Dictionary<Anchor, Dictionary<string, object>>();
        ...
        public static void DebugValue(string key, object value, Anchor anchor = Anchor.UpperRight)
        {
            if (!Instance._values.ContainsKey(anchor))
                Instance._values.Add(anchor, new Dictionary<string, object>());

            if (Instance._values[anchor].ContainsKey(key))
                Instance._values[anchor][key] = value;
            else
                Instance._values[anchor].Add(key, value);
        }
        ...
        private void OnGUI()
        {
            foreach (KeyValuePair<Anchor, Dictionary<string, object>> values in _values)
            {
                Anchor anchor = values.Key;
                int i = 0;
                
                foreach (KeyValuePair<string, object> entry in values.Value)
                {
