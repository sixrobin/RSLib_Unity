namespace RSLib
{
	...
	public class IKFeetPlacement : MonoBehaviour
	{
		...
		private float _heightFromGroundRaycast = 1.15f;
		[SerializeField, Range (0f, 2f)]
		private float _raycastsDistance = 1.5f;
		[SerializeField, Range (0f, 1f)]
		private float _pelvisVerticalSpeed = 0.28f;
		[SerializeField, Range (0f, 1f)]
		...
		[Space (15)]
		[SerializeField]
		private bool _useProIKFeature = false;
		[SerializeField]
		private bool _showSolverDebug = true;
		...
		{
			if (_showSolverDebug)
				UnityEngine.Debug.DrawLine(fromSkyPosition, fromSkyPosition + Vector3.down * (_raycastsDistance + _heightFromGroundRaycast), Color.green);
